...
int main(int argc, char *argv[]) {
  ...
  std::set<std::string> meshtypes = {"uniform", "rectilinear", "structured",
                                     "unstructured"};
  ...
  std::cout << "Creating mesh of type " << meshtype << " of resolution "
            << grid_resolution << "x" << grid_resolution << std::endl;
  simulation_data sim = {.resolution = grid_resolution, .mesh = meshtype};
  SimInitialize(&sim);

  sim.cart_dims[0] = sim.cart_dims[1] = 0;
  int PartitioningDimension =
      2; // want a 2D MPI partitioning. otherwise set to 1.
  MPI_Init(&argc, &argv);
  ...
  // We use (bx + 2) grid points in the X direction, i.e. interior points plus 2
  // b.c. points We use (by + 2) grid points in the Y direction, i.e. interior
  // points plus 2 b.c. points decompose the domain
  ...
  while (sim.gdel > TOL) {
    ...
    CatalystAdaptor::Execute(
        sim); // sim.iter*0.1, sim.iter*0.1, grid, attributes);
#endif
#ifdef USE_ASCENT
  ...
  if (!sim.par_rank)
    std::cout << "Stopped at iteration " << sim.iter
              << " . Maximum error = " << sim.gdel << std::endl;
